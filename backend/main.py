...
def generate_video_with_runway(prompt):
    ...
    print(f"Generating video with RunwayML Gen-2: {prompt}")
    
    # RunwayML Gen-2 API endpoint (corrected)
    ...
    # Updated payload format according to RunwayML's latest API documentation
    data = {
        "input": {
            "prompt": prompt,
            "negative_prompt": "Bad quality, blurry, low resolution, disfigured",
            "aspect_ratio": "9:16"  # Vertical/portrait for Instagram reels
        }
    }
    
    try:
        # Make initial request to start generation
        print(f"Making RunwayML API request with data: {json.dumps(data)}")
        response = requests.post(url, json=data, headers=headers)
        
        print(f"RunwayML response status: {response.status_code}")
        print(f"RunwayML response: {response.text}")
        
        response.raise_for_status()
        result = response.json()
        print(f"Full response from RunwayML: {json.dumps(result)}")
        
        # Check if we have a video URL directly in the response
        if "output" in result and "videos" in result["output"] and len(result["output"]["videos"]) > 0:
            video_url = result["output"]["videos"][0]
            return video_url
            
        # Otherwise, check for generation ID to poll
        generation_id = result.get("id")
        if not generation_id:
            raise ValueError("No generation ID in RunwayML response")
        
        print(f"RunwayML generation started with ID: {generation_id}")
        
        # Poll for the result
        video_url = poll_runway_result(generation_id)
        return video_url
    except Exception as e:
        print(f"Error generating video with RunwayML: {e}")
        raise

def poll_runway_result(generation_id, max_attempts=60, delay=5):
    """Poll RunwayML API for the result of a video generation request"""
    url = f"https://api.runwayml.com/v1/inference/{generation_id}"
    ...
    for attempt in range(max_attempts):
        try:
            print(f"Polling RunwayML result, attempt {attempt+1}/{max_attempts}")
            response = requests.get(url, headers=headers)
            
            print(f"Poll response status: {response.status_code}")
            if response.status_code == 404:
                print(f"Generation {generation_id} not found, waiting...")
                time.sleep(delay)
                continue
                
            response.raise_for_status()
            result = response.json()
            print(f"Poll result: {json.dumps(result)}")
            
            status = result.get("status")
            print(f"RunwayML status: {status}")
            
            if status == "succeeded":
                # Get the video URL from the result
                if "output" in result and "videos" in result["output"] and len(result["output"]["videos"]) > 0:
                    return result["output"]["videos"][0]
                elif "output" in result and isinstance(result["output"], str):
                    return result["output"]
                else:
                    print(f"Unexpected result format: {result}")
                    raise ValueError(f"No video URL in completed result: {result}")
            elif status == "failed":
                raise ValueError(f"RunwayML generation failed: {result.get('error', 'Unknown error')}")
            elif status in ["starting", "processing"]:
                # Still processing, wait and try again
                time.sleep(delay)
                continue
            else:
                raise ValueError(f"Unknown status from RunwayML: {status}")
        except Exception as e:
            if isinstance(e, ValueError):
                raise
            print(f"Error polling RunwayML: {e}")
            # If there's an API error, wait and try again
            time.sleep(delay)
    
    raise TimeoutError(f"RunwayML generation timed out after {max_attempts} attempts")

def process_reel(job_id, prompt, voice_script):
    """Process a reel job with video generation, voiceover, and subtitle overlay"""
    try:
        reel_jobs[job_id]['status'] = 'generating_video'
        
        # Step 1: Generate video with RunwayML
        video_url = generate_video_with_runway(prompt)
        reel_jobs[job_id]['video_url'] = video_url
        
        # If no voice script is provided, use the prompt
        if not voice_script:
            voice_script = f"This is a video about {prompt}"
        
        reel_jobs[job_id]['status'] = 'generating_voiceover'
        
        audio_content = text_to_speech(voice_script)
        
        # Create temporary files
        with tempfile.NamedTemporaryFile(suffix='.mp4', delete=False) as video_file, \
             tempfile.NamedTemporaryFile(suffix='.mp3', delete=False) as audio_file, \
             tempfile.NamedTemporaryFile(suffix='.srt', delete=False) as srt_file, \
            ...
            srt_content = generate_srt_from_text(voice_script)
            srt_file.write(srt_content.encode('utf-8'))
            srt_file.flush()
            
            reel_jobs[job_id]['status'] = 'processing_ffmpeg'
            
            # Step 3: Use FFmpeg to merge video, audio, and add subtitles
            ...
            # Save the output file to a static location
            output_path = f"./temp_videos/{job_id}.mp4"
            os.makedirs(os.path.dirname(output_path), exist_ok=True)
            
            with open(output_file.name, 'rb') as f_in, open(output_path, 'wb') as f_out:
        ...
        print(f"Error processing reel job {job_id}: {e}")
        reel_jobs[job_id]['status'] = 'failed'
        reel_jobs[job_id]['error'] = str(e)

@app.post("/generate-reel")
async def generate_reel_endpoint(request: ReelRequest, background_tasks: BackgroundTasks):
    """Endpoint to generate a reel with video, voiceover, and subtitles"""
    try:
        ...
        # Process the reel generation in the background
        background_tasks.add_task(process_reel, job_id, request.prompt, request.voice_script)
        
        return {"job_id": job_id, "status": "queued"}
    
    except Exception as e:
        print(f"Error in generate_reel_endpoint: {e}")
...
async def chat_endpoint(request: ChatRequest):
    try:
        ...
        if is_video_request:
            ...
            # Return the text response directly, not as HTML
